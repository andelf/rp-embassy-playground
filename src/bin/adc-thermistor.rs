...
async fn main(_spawner: Spawner) {
    ...
    loop {
        let level = adc.read(&mut p26).await; // 12 bit ADC, max value is 4095
        let raw_10bit = level >> 2;

        let temp = rp::thermistor::analog_to_temp(raw_10bit);

        info!("thermistor: {} {}℃", raw_10bit, temp);
